...
const PopularProducts = ({item, navigation, setVisible, showModal, setActiveData}) => {
  return (
    <TouchableOpacity
      onPress={() => {
  ...
  return (
    <SafeAreaView style={{...GlobalStyle.Container, paddingHorizontal: 0}}>
      <View style={{paddingHorizontal: 20}}>
        <AppBar theme="dark" title="My Orders" />
      </View>
      ...
      <View
        ...
        <FlatList
          ...
          horizontal={true}
          renderItem={({item}) => <MenuItem item={item} setActive={setActive} />}
        ...
        {loading ? (
          ...
          <FlatList
            ...
            data={active == 'any' ? orderList : orderList.filter(x => x.status === active)}
            renderItem={({item}) => (
              <PopularProducts
                visible={visible}
